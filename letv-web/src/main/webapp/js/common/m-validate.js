...
var MValify = {
		//检验是否为空
		isNull: function(str, id, t, l,j){
			if(!str){
				AlertDialog.show($("#" + id).attr("nullMessage"), id, t, l);
				if(j){
		...
		username: function(str, result, id){
			...
			if(!str){
				//AlertDialog.show("用户名不能为空", id, -5, 100);
				$("#"+id).text('用户名不能为空').css("visibility","visible");
				return false;
			}else if(/[\u4E00-\u9FA5]/i.test(str)){
				//AlertDialog.show("用户名不能含有中文", id, -5, 100);
				$("#"+id).text('用户名不能含有中文').css("visibility","visible");
				return false;
			}else if(!isNaN(str)){
				//AlertDialog.show("用户名不能为纯数字", id, -5, 100);
				$("#"+id).text('用户名不能为纯数字').css("visibility","visible");
				return false;
			}else if(str.length < 6 || str.length > 16){
				//AlertDialog.show("用户名长度应该为6到16位", id, -5, 100);
				$("#"+id).text('用户名长度应该为6到16位').css("visibility","visible");
				return false;
			}else{
				//如果验证用户名格式通过，验证是否重复
				$.ajax({
					...
					success: function(data){
						if(data["msg"] == "success"){
							...
							$("#"+id).text(data["msg"]).css("visibility","visible");
							result = false;
						}
					},
					error: function(){
		...
		password: function(str,id){
			if(!str){
				AlertDialog.show("密码长度应该为6~16位", id, -5, 130);
				return false;
			}
		...
		rePass: function(str, str1, id){
			if(!str1){
				AlertDialog.show("密码不能为空", id, -5, 130);
				return false;
			}
			if(str != str1){
				AlertDialog.show("两次输入密码不一致", id, -5, 130);
				return false;
			}else{
				AlertDialog.hide();
				return true;
		...
		valiCode: function(str, result, id, isAjax){
			var $tip = $("#" + id);
			if(!str){
				$tip.text("图片验证码不能为空").css("visibility","visible");
				return false;
			}
			if(isAjax == false){
				return result;
			...
			$.ajax({
				...
				success: function(data){
					if(data["msg"] == "success"){
						...
						$tip.text(data["msg"]).css("visibility","visible");
		...
		phone: function(phone, id){
			...
			if(isPhoneReg.exec(phone)){
				//AlertDialog.hide();
				$("#"+id).text("手机号不能为空").css("visibility","hidden");
				return true;
			}else{
		...
		postCode: function(code, id){
			...
			var $inputt = $("#" + id);
			if(!code){
				$inputt.text("请输入邮编").css("visibility","visible");
				return false;
			}
			if(code.length != 6){
				$inputt.text("请输入正确的邮编").css("visibility","visible");
				return false;
			}
			if(!isNaN(code)){
		...
		realName: function(name, id){
			...
			if(!name){
				AlertDialog.show("真实姓名不能为空", id, 0, 150);
				return false;
			}
			l = name.length;
			for(var i=0;i<l;i++){
				...
				if((nameCharCode > 255 && nameCharCode < 40869) || nameCharCode == 183){//中文
					...
					AlertDialog.show("真实姓名输入不正确", id, 0, 150);
					return false;
				}
			}
		...
		cardCode: function(code, id){
			if(!code){
				AlertDialog.show("证件号码不能为空", id, 0, 150);
				return false;
			}else{
				var num = code.toUpperCase();  
				//身份证号码为15位或者18位，15位时全为数字，18位前17位为数字，最后一位是校验位，可能为数字或字符X。
				if (!(/(^\d{15}$)|(^\d{17}([0-9]|X)$)/.test(num))) {
					AlertDialog.show("身份证号码不正确", id, 0, 150);
					return false;
				}

var loanNo, loanState,loanCover,loanType; //预定义项目编号和评论过后的标识
...
//		});
		...
		if(siteUserId == "null"){
			$(".header1>div.box").html('<div class="fl"><p>欢迎您来到乐视金融众筹网！</p></div><div class="fr"><i><a style="padding:0 7px;" class="red" href="'+path+'/common/login.html" >登录</a><a href="'+path+'/common/register.html">注册</a></i><span class="line">|</span><a target="_blank">乐视金融</a><span class="line">|</span><a>帮助中心</a><span class="line">|</span><em>热线电话：400-999-5157</em></div>');
		}else{
...
function crowdfund(){
	if(siteUserId == "null"){
		...
		AlertDialog.confirm(loginUrl, null, "您还没有登录，请登录！", "去登录", "再看看", null);
		return false;
	}
	$.ajax({
	...
	$.ajax({
		...
		success: function(data){
			...
			if(data["loanState"] == "preheat"){
				$("#preheatB").show().find("#overFundAmt").text(data["overFundAmt"].toFixed(2));
				$("#blockChain-p").hide();
			}
			if(data["loanState"] == "preheat"){
				$("#preheatA,#preheatC,#preheatD").hide();
			}
			...
			$("#loanUser").text(data["loanUser2"]).attr("loanUser", data["loanUser"]);
...
//获取回报设置列表
function getBackSetList(loanState){
		...
		success: function(data){
			...
			for(var i=0;i<l;i++){
				if(data[i]["backLable"] != "M"){
					...
					if(data[i]["prizeDrawFlag"]=="1"){
						bArr.push('<p class="mtb10 ft14" style="color:#44b4ff;cursor: pointer;" id="lotteryRuleBlock">抽奖规则>> </p>');
					}
					bArr.push('<p class="mtb10 ft_bd ft14">你将获得： </p>');
					bArr.push('<div class="lg26">');
					if(data[i]["prizeDrawFlag"]=="1"){
						bArr.push('该档位共有'+data[i]["prizeNum"]+'个中奖名额，总限投人数'+data[i]["numberLimits"]+'位，每人最高限投'+data[i]["prizeInvestNum"]+'次。满足'+data[i]["prizeFullNum"]+'位支持者即可进行抽奖，不满足时不进行抽奖。');
					}
					bArr.push(data[i]["backContent"]+'</div>');
					if(data[i]["photoUrl1"]){
					...
					if(siteUserId == "null"){
						...
						if(loanState == "funding"){ //筹款中
							if(state == "notfull"){
								...
								var prizeDrawFlag = data[i]["prizeDrawFlag"];
								var prizeInvestNum = data[i]["prizeInvestNum"];
								var supportCount = data[i]["supportCount"];
								$.ajax({
									...
									success: function(data){
										if(loanUser == siteUserId){
											...
											if(data["isAuth"]){
												if(prizeDrawFlag == "1" && Number(supportCount)>=Number(prizeInvestNum)){
													bArr.push('<a onclick="supportCount()" class="agree_btn" >支持'+supportAmt+companyCode+'</a>');
												}else{
													
													bArr.push('<a href="'+path+'/common/support-pubBenefit.html?loanNo='+loanNo+'&backNo='+supportBackNo+'&ctype=1"  target="_blank" class="agree_btn" >支持'+supportAmt+companyCode+'</a>');
												}
											}else{
			...
			$("#lotteryRuleBlock").click(function(){
				$("#passwordDiv,#bgpop2").show();
				$(".prompt_close").click(function(){
					$("#passwordDiv,#bgpop2").hide();
				})
			});
...
//			pArr.push('<span class="colaa">'+data[i]["discussUser"]+'：</span>'+data[i]["content"]+'');
			...
			pArr.push('<textarea placeholder="评论不能少于5个字哦！" class="reply"  id="'+data[i]["id"]+i+'"> </textarea>');
...
//				pArr.push('<span class="colaa">' + Rdata[j]["discussUser"] +'：</span>' + Rdata[j]["content"] +'');
		...
		$(".replay_content textarea.reply").keyup(function(){
			if($(this).val().length>140){
				AlertDialog.show("不能超过140个字！", $(this).attr("id"), -20, 30, "jump");
				$(this).val($(this).val().substring(0,140));
			}
		})
		//刷新后改变评论数量
...
//			pArr.push('<span class="colaa">' + Rdata[j]["discussUser"] +'：</span>' + Rdata[j]["content"] +'');
	...
	$(".replay_content textarea.reply").keyup(function(){
		if($(this).val().length>140){
			AlertDialog.show("不能超过140个字！", $(this).attr("id"), 0, 30, "jump");
			$(this).val($(this).val().substring(0,140));
		}
	})
	//分页设置

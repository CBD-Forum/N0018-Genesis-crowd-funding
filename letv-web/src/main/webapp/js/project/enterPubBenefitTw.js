	...
	$("#completeBtu").click(function(){
		if($("#soprtList>li").length>0){
			if(Number($("#priceAmt").val())>Number($("#overFundAmt").val())){
				AlertDialog.tip("您的回报总金额超出最高上限金额，请调整！");
			}else{
				if(Number($("#priceAmt").val())<Number($("#overFundAmt").val())){
					AlertDialog.confirmReward(enterPubBenefitUrl,null,"项目已保存回报金额小于设置最高上限金额，您确定提交吗？","确定提交","继续添加",null);
				}else{
					window.location.href = path+"/common/enterPubBenefitTr.html?loanNo="+loanNo
				}
			}
...
//		if(Number($("#price").val())*limitPeople>=surplusAmt){

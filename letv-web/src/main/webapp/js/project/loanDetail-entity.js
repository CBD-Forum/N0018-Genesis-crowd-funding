...
function getCrowdDetail(){
	...
	$.ajax({
		...
		success: function(data){
			...
			$("#fundAmt").text(formatCurrency(data["fundAmt"]));
...
function getBackSetList(){
	var tArr = [], tStr = '',tArr2=[],tStr='', l; //顶部两条列表展示
				...
				if(i < 3){
					...
					if(siteUserId == "null"){
						...
						if(loanState == "funding"){ //筹款中
							if(data[i]["state"] == "notfull"){
								tArr.push('<a href="'+path+'/common/support-entity.html?loanNo='+loanNo+'&backNo='+data[i]["backNo"]+'" style="" target="_blank">支持¥'+data[i]["amt"]+'</a>');
							}else if(data[i]["state"] == "full"){
								tArr.push('<a href="javascript:void(0);" style="background:#ccc">支持¥'+data[i]["amt"]+'</a>');
							}
						}else{
...
//					tArr.push('<p class="p1">支持'+wordNum[i]+'      '+data[i]["backContent"]+'</p>');
			...
			for(var i=0;i<l;i++){
				...
				if(siteUserId == "null"){
					...
					if(loanState == "funding"){ //筹款中
						if(data[i]["state"] == "notfull"){
							aArr.push('<p class="jlzc_rgje" style="float:right;margin-top:-60px;"><a href="'+path+'/common/support-entity.html?loanNo='+loanNo+'&backNo='+data[i]["backNo"]+'" target="_blank">支持￥'+data[i]["amt"]+'</a></p>');
						}else{
			...
			for(var i=0;i<l;i++){
				...
				if(siteUserId == "null"){
					...
					if(loanState == "funding"){ //筹款中
						if(state == "notfull"){
							bArr.push('<span class="rgq"><a href="'+path+'/common/support-entity.html?loanNo='+loanNo+'&backNo='+data[i]["backNo"]+'" target="_blank">支持￥'+data[i]["amt"]+'</a></span>');
						}else if(state == "full"){
							bArr.push('<span class="rgq" style="background:#ccc"><a href="javascript:void(0);">支持￥'+data[i]["amt"]+'</a></span>');
						}
					}else{

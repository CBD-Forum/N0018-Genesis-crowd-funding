...
$(function(){
	if(tcode == "lended"){
		...
		$("#tabTit>li").eq(0).find("a").text("未参与中奖发货");
		$("#tabTit>li").eq(1).find("a").text("中奖发货");
	}else{
		$("#deliveryTitle").text("发货明细");
		$("#tabTit>li").eq(0).find("a").text("未参与中奖发货明细");
		$("#tabTit>li").eq(1).find("a").text("中奖发货明细");
	}
	if(ctype == "1"){
	...
	$.each($tab,function(k,v){
		$(v).click(function(){
			...
			getLogicoList(1, $(this).attr("vname"));
		});
	});
	$tab.first().click();
...
function ajaxPageData(obj){
	...
	var vname = $("#tabTit>li>a.a_home").attr("vname");
	getLogicoList(obj["current"], vname);
}
//展示录入物流信息弹出框
...
//							$("a[eid=\""+bid+"\"]").text("已发送");	
							getLogicoList(1, $("#tabTit>li>a.a_home").attr("vname"));
						}else{

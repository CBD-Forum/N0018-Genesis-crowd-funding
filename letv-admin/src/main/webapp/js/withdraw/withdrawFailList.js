$(function(){
	...
	var columns = [[
					...
					{field:'memo',title:'提现失败',width:250,align:'right'} 
				/*	{field:'feeAcctNo',title:'手续费支付方',width:150,align:'center'},
	...
	var exportBtn = { 
	        ...
	        handler: function() {
	        	var params = {};
	        	params.state='passed';
	        	commonExportExcel('finance_passed_withdraw_export', params);
	        } 
	      };
	var operateBtns=[];
	if ($('#finance_passed_withdraw_export').length>0) {
	...
	$('#withdrawTable').datagrid({
		url: path + "/withdraw/getlist.html?state=withdraw_fail",
		columns: columns,
		pagination: true,
		singleSelect: true,

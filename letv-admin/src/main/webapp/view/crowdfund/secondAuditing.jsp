...
%>
<script type="text/javascript" src="<%=path%>/js/crowdfund/secondAuditing.js"></script>
<script type="text/javascript" src="<%=path%>/js/crowdfund/loanCommon.js"></script>
	...
	<div id="add">
		<form id="auditform" method="post" style="margin-top:20px;">
			...
			<div id="auditTextDiv"></div>
			<div class="psb" style="margin-top:30px;"><a id="pass" class="easyui-linkbutton searchBtn">复审通过</a><a id="refuse" class="easyui-linkbutton searchBtn">复审拒绝</a></div>
		</form>
	</div>
	...
	function operateData(val,row,index){
		var returnStr = '';
		//审核
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_reauditing_audit\')"><a onclick=operate("'+row["loanNo"]+'","'+row["loanType"]+'")>审核</a></security:authorize>';
		//编辑
		returnStr+= '<security:authorize access="hasPermission(null, \'security.operation.crowdfund_reaudit_edit\')"><a onclick=editLoan("'+row["loanNo"]+'","'+row["loanType"]+'")>编辑</a></security:authorize>';
		return returnStr;
	}
	...
	function operateData1(val,row,index){
		var returnStr = '';
		//项目详情
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_reauditing_detail\')"><a onclick=LoanDetail("'+row["loanNo"]+'","'+row["loanType"]+'")>项目详情</a></security:authorize>';
		//项目回报
	    if("stock"!=row["loanType"]){
			returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_reauditing_back\')"><a onclick=openBackSet("'+row["loanNo"]+'","'+row["loanType"]+'")>项目回报</a></security:authorize>';
	    }
		//审批流程
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_reauditing_auditList\')"><a onclick=openAudit("'+row.loanNo+'")>审核流程</a></security:authorize>';
		//预览
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_reauditing_preview\')"><a onclick=openPreview("'+row["loanNo"]+'","'+row["loanType"]+'","'+row["loanState"]+'")>预览</a></security:authorize>';
		return returnStr;
	}
	</script>
...
%>
<script type="text/javascript" src="<%=path%>/js/crowdfund/releasing.js"></script>
<script type="text/javascript" src="<%=path%>/js/crowdfund/loanCommon.js"></script>
		...
		<form id="auditform" method="post" style="margin-top:20px;">
			...
			<div class="x-form-item" id="sDate">
				...
				<div class="x-form-element">
					<input type="text" name="fundEndTime" class="Wdate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" style="margin-left:0; width:240px;"/>
				</div>
			</div>
	...
	function operateData(val,row,index){
		var returnStr = '';
		//编辑
		returnStr+= '<security:authorize access="hasPermission(null, \'security.operation.crowdfund_releasing_edit\')"><a onclick=editLoan("'+row["loanNo"]+'","'+row["loanType"]+'")>编辑</a></security:authorize>';
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_releasing_release\')"><a onclick=release("'+row["loanNo"]+'")>发布</a></security:authorize>';
		return returnStr;
	}
	...
	function operateData1(val,row,index){
		...
		//项目回报
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_releasing_back\')"><a onclick=openBackSet("'+row["loanNo"]+'","'+row["loanType"]+'")>项目回报</a></security:authorize>';
		//审批流程

...
%>
<script type="text/javascript" src="<%=path%>/js/crowdfund/sended.js"></script>
<script type="text/javascript" src="<%=path%>/js/crowdfund/loanCommon.js"></script>
	...
	function operateData1(val,row,index){
		var returnStr = '';
		//支持列表
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_sended_support\')"><a onclick=supportList("'+row["loanNo"]+'","'+row["loanType"]+'")>支持列表</a></security:authorize>';
		//项目详情
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_sended_detail\')"><a onclick=LoanDetail("'+row["loanNo"]+'","'+row["loanType"]+'")>项目详情</a></security:authorize>';
		//项目回报
		returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_sended_back\')"><a onclick=openBackSet("'+row["loanNo"]+'","'+row["loanType"]+'")>项目回报</a></security:authorize>';
		
		if(row["loanType"]=='product'){
			returnStr+='<security:authorize access="hasPermission(null, \'security.operation.crowdfund_sended_prize\')"><a href="'+path+'/crowdfund.crowdfundUserPrizeList.html?loanNo='+row["loanNo"]+'">抽奖列表</a></security:authorize>';	
		}
		return returnStr;
	}
	</script>